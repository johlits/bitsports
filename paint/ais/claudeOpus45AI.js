 ...
 * - Early game: Rapid territory expansion with efficient sweep patterns
 * - Mid game: Powerup hunting + strategic painting in dense clusters
 * - Late game: Aggressive tile stealing when behind, defensive when leading
...
function evaluatePowerup(powerup, self, others, timeRemaining) {
  ...
  switch (powerup.type) {
    ...
    case "speed":
      // 1.5x speed for 3 seconds = ~36 extra units of movement
      value = 10;
      if (timeRatio > 0.6) value *= 1.3; // Early game = more tiles
      if (self.powerups.speedBoost > 1) value *= 0.2; // Already boosted
      break;

    case "shield":
      // 5 seconds of protection - valuable when leading
      value = 6;
  ...
  // Full value at 0, 50% at distance 8
  const distPenalty = Math.min(distance / 16, 0.7);
  value *= (1 - distPenalty);

  // Check if opponent is closer - reduce value if they'll get it first
...
/**
 * Count paintable tiles (unpainted or enemy) in a region
 */
...
/**
 * Find best tile to paint using cluster scoring
 */
  ...
  for (let radius = 1; radius <= maxRadius; radius++) {
    for (let dx = -radius; dx <= radius; dx++) {
      for (let dy = -radius; dy <= radius; dy++) {
        ...
        // Cluster bonus: more paintable tiles nearby = better
        ...
        // Sweep pattern bonus: prefer continuing in same direction
        if (lastSweepY >= 0) {
...
export function tick(state) {
  ...
  // Infer our ID from current tile if painted
  const myId = grid[selfGX]?.[selfGY] || 0;
  ...
  if (target) {
    ...
    if (len < 0.25) {
      ...
      const nextGX = target.gx + sweepDir;
      const nextGY = target.gy;
      if (nextGX >= 0 && nextGX < GRID_SIZE) {
        if (grid[nextGX][nextGY] === 0 || grid[nextGX][nextGY] !== myId) {
          ...
          sweepDir *= -1;
          const newY = target.gy + 1;
          if (newY < GRID_SIZE) {
            const next = gridToWorld(target.gx, newY);
            return normalize(next.x - self.x, next.z - self.z);
          }
        }
      }
