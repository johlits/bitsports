...
function chooseShotTargetX(puck, opponent) {
  const goalHalf = GOAL_WIDTH / 2 - 0.2;
  const awayFromOpponent = opponent.x >= 0 ? -1.8 : 1.8;
  const sideBias = clamp(-puck.x * 0.35, -1.4, 1.4);
  const mixed = awayFromOpponent * 0.7 + sideBias * 0.3;
  return clamp(mixed, -goalHalf, goalHalf);
}

function approachBehindPuck(puck, targetX, targetZ, extra = 0.24) {
...
function analyzePuck(puck, self, opponent, isBlue) {
  ...
  let threatScore = 0;
  if (movingTowardsMe) {
    if (timeToGoal < Infinity) threatScore += 130 / (timeToGoal + 0.08);
    if (onTarget) threatScore += 55;
    threatScore += speed * 4.5;
    threatScore += clamp(9 - distToGoal, -5, 9) * 3;
  }

  if (inMyHalf) threatScore += 18;
  threatScore += clamp(6 - selfDist, -6, 6) * 1.2;

  if (!inMyHalf && oppDist < 2.0) threatScore += 12;
...
function chooseOffenseTarget(analyses, self, isBlue) {
  ...
  for (const a of analyses) {
    ...
    if (a.movingTowardsMe && a.timeToGoal < 1.6) continue;
    if (a.speed > 4.8) continue;

    const dist = Math.hypot(a.puck.x - self.x, a.puck.y - self.y);
    if (dist > 6.5) continue;

    const score = (6.5 - dist) + (4.8 - a.speed) + (a.movingTowardsMe ? -1.2 : 1.8);
    if (score > bestScore) {
      bestScore = score;
      best = a;
    }
...
function getDefensePosition(primary, isBlue) {
  const defenseZ = isBlue
    ? HALF_H - (GOAL_CREASE_RADIUS + PADDLE_RADIUS + 0.55)
    : -HALF_H + (GOAL_CREASE_RADIUS + PADDLE_RADIUS + 0.55);
  const pressZ = isBlue ? 2.8 : -2.8;
  const guardZ = primary.speed > 4.2 || primary.movingTowardsMe ? defenseZ : pressZ;
  const pred = predictAtZ(primary.path, guardZ, isBlue);
  return {
    x: clamp(pred.x, -2.6, 2.6),
    z: guardZ,
  };
}
...
export function tick({ pucks, self, opponent }) {
  ...
  if (!primary) {
    ...
    const critical = analyses.filter(
      (a) => a.movingTowardsMe && a.onTarget && a.timeToGoal < 1.1
    );
  ...
  if (dir.x === 0 && dir.z === 0) {
    const fallback = getValidPaddlePosition(0, isBlue ? myGoalZ - 4.0 : myGoalZ + 4.0, isBlue);
